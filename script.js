document.addEventListener('DOMContentLoaded', () => {
  ...
  if (slider && magValue && pressureValue && chartRoot) {
    const magnitudes = [];
    const pressures = [];
    ...
    svg.appendChild(make('rect', {
      x: M.left, y: M.top, width: plotW, height: plotH, rx: 18,
    ...
    for (let i = 0; i <= yTicks; i++) {
      ...
      const label = make('text', {
        x: M.left - 12, y: y + 4, 'text-anchor': 'end',
        fill: '#a4b3ca', 'font-size': 13
      });
    ...
    xTicks.forEach((xVal) => {
      ...
      const label = make('text', {
        x, y: M.top + plotH + 28, 'text-anchor': 'middle',
        fill: '#a4b3ca', 'font-size': 13
      });
    ...
    const xAxisLabel = make('text', {
      x: M.left + plotW / 2, y: H - 12, 'text-anchor': 'middle',
      fill: '#a4b3ca', 'font-size': 14, 'font-weight': 600
    });
    xAxisLabel.textContent = 'MAG (mV)';
    svg.appendChild(xAxisLabel);

    const yAxisLabel = make('text', {
      x: 20, y: M.top + plotH / 2, 'text-anchor': 'middle',
      fill: '#a4b3ca', 'font-size': 14, 'font-weight': 600,
    ...
    const areaData = `${pathData} L ${xScale(xMax)} ${M.top + plotH} L ${xScale(xMin)} ${M.top + plotH} Z`;
    ...
    const pointLineX = make('line', {
      stroke: 'rgba(244, 114, 182, 0.35)', 'stroke-width': 1.5, 'stroke-dasharray': '6 5'
    });
